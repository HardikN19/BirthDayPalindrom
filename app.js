...
var daysInMonth=[31,Number(`${new Date().getFullYear() % 4 ? 29 : 28}`),31,30,31,30,31,31,30,31,30,31];
...
function inputManager(){
    ...
    if(result){
        output.innerText=`Wow!!! Your birthdate in format ${result} is a palindrome.`;
    }else{
        let newData=nextPalndrome(dateStr,monthStr,yearStr);
        let nearestPal=newData[0]
        let daysAway=newData[1]
        output.innerText=`No! Your birthdate is not palindrome. Nearest palindrome date is ${nearestPal} You missed it by ${daysAway} days.`
    }
}
...
function nextPalndrome(dd,mm,yyy){
    ...
    while (true) {
        ...
        if (nextDate > daysInMonth[nextMonth - 1]) {
            nextDate = 1;
            nextMonth += 1;
            if (nextMonth > 12) {
                nextMonth = 1;
        ...
        if (prevDate < 1) {
            ...
            if (prevMonth < 1) {
                ...
                if(prevYear < 1) {
                    ...
                    prevMonth = 12;
                    prevDate = daysInMonth[prevMonth - 1];
                }
            } else {
            prevDate = daysInMonth[prevMonth - 1];
            }
        ...
        const nextPal=checkAllFormates(nextDate,nextMonth,nextYear);
        if (nextPal)
            return [nextPal, missedDays];

            const prevPal=checkAllFormates(prevDate,prevMonth,prevYear);
        if (prevPal)
            return [prevPal, missedDays];
    }
}